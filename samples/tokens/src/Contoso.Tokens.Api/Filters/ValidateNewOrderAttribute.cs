namespace Contoso.Services.Filters
{
    ...
    public sealed class ValidateNewOrderAttribute : ActionFilterAttribute
    {
        public override async Task OnActionExecutingAsync( HttpActionContext actionContext, CancellationToken cancellationToken )
        {
            ...
            {
                result = actionContext.BadRequest( nameof( InvalidOrderItem ), InvalidOrderItem );
            }
            else if ( !options.IsValidField( o => o.Quantity, modelState ) )
            {
                result = actionContext.BadRequest( nameof( InvalidOrderQuantity ), InvalidOrderQuantity );
            }
            else
            {
                return;
